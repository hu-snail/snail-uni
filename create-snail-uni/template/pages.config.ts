...
export default defineUniPages({
  ...
  easycom: {
    ...
    custom: {
      ...
      '^nut-(.*)': 'nutui-uniapp/components/$1/$1.vue'<% } %><% if (uiType === 'UniUI') { %>'^uni-(.*)': '@dcloudio/uni-ui/lib/uni-$1/uni-$1.vue'<% } %><% if (uiType === 'Sard-ui') { %> "^sar-(.*)": "sard-uniapp/components/$1/$1.vue"<% } %><% if (uiType === 'Uview-Pro') { %> "^u-(.*)": "uview-pro/components/u-$1/u-$1.vue"<% } %>
    },
  }<% if (useTabbar) { %>,
  tabBar: {
    color: '#999999',
